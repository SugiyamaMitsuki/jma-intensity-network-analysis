...
date: "2026-05-09"
authors: "Codex; Mitsuki Sugiyama"
---
...
- Resampling recent intensity-5-upper-or-larger events to the 1994 year-end station geometry lowers the sampled maximum intensity for most events, confirming that maximum-intensity statistics depend strongly on station geometry.
...
Japan reports earthquake shaking using the JMA seismic-intensity scale. The number of intensity meters increased greatly after the 1995 Kobe earthquake. This makes the catalog more useful for emergency response, but it also changes the statistics: modern earthquakes are more likely to have a station close to the epicenter, so the maximum reported intensity is more strongly affected by network geometry than in the older sparse network. When recent strong events are resampled to the station layout that existed at the end of 1994, most of them would have had a lower sampled maximum intensity. At the same time, the average over reporting stations can become smaller because many distant low-shaking stations are included. We also tested how station density affects intensity mapping. A simplified distance-attenuation baseline alone matches the exact JMA intensity class at only about 46% of withheld stations. Interpolation from observations reaches about 70% exact class agreement for all withheld reporting stations at roughly 58 stations per 10,000 km2, but this does not mean that strong-shaking areas are reliably reconstructed. For stations with intensity 5 lower or larger, the hit rate remains around 45% even at the highest tested density.
...
3. Resampling recent events to the 1994 year-end station geometry directly evaluates maximum-intensity sampling by network geometry.
4. All-withheld-station exact class agreement captures interpolation errors that are not visible from RMSE alone.
5. Prediction accuracy differs among broad station-region groups because station density, source geometry, and surface-site variability interact.
...
### 3.6 Maximum-intensity counterfactual under the 1994 year-end station geometry

To evaluate how recent observed maximum intensities would change under a historical station geometry, we resampled 85 events in 2011-2022 with maximum JMA intensity 5 upper or larger to the station geometry active at 23:59 JST on 31 December 1994. For each 1994 year-end station, we searched the current reporting stations for the same event and assigned a pseudo-observation only if the nearest current report was within 10 km. The 1994-geometry pseudo-maximum is the maximum of those assigned pseudo-observations. This procedure evaluates maximum-intensity sampling by station geometry; it does not reconstruct official 1994 observations, intensity-zero-equivalent non-reports, instrument response, or site installation conditions.
...
### 4.10 Maximum intensity of recent strong events under the 1994 year-end station geometry

Beyond the single northern Osaka case, we evaluated how observed maximum intensity for recent earthquakes would change under the 1994 station geometry. The target set contains 85 events in 2011-2022 with maximum JMA intensity 5 upper or larger. The counterfactual network consists of the 450 intensity stations active at 23:59 JST on 31 December 1994. For each 1994 year-end station, we assigned a pseudo-observation only when the nearest current reporting station for the event lay within 10 km; the assigned value was that nearest current observed intensity. This is a resampling experiment for station geometry and does not reproduce historical instrument response, site installation, operational processing, or unreported intensity-zero stations.

Across the 85 events, the median current observed maximum intensity is 5.20, whereas the median pseudo-maximum under the 1994 year-end geometry is 4.50. The median reduction is 0.90 intensity units; 71.8% of the events decrease by at least 0.5 units and 43.5% decrease by at least 1.0 unit. In class terms, 83.5% of the events drop by at least one JMA intensity class. Restricting the sample to the 78 events with at least 100 current reports and at least 20 matched 1994-geometry pseudo-observations gives the same median reduction of 0.90, indicating that the result is not driven only by sparsely reported events. The median nearest reporting-station distance is 6.97 km in the current reports but 18.57 km for matched pseudo-observations under the 1994 geometry, demonstrating the change in geometric sampling of the strongest shaking.

**Table 9. Maximum-intensity changes when recent 2011-2022 intensity-5-upper-or-larger events are resampled to the 1994 year-end station geometry.**

| Target subset | Events | Current max median | 1994-geometry pseudo-max median | Median reduction | Reduction >=0.5 | Reduction >=1.0 | Class drop >=1 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| All target events | 85 | 5.20 | 4.50 | 0.90 | 71.8% | 43.5% | 83.5% |
| Current >=100 reports and 1994 geometry >=20 matches | 78 | 5.25 | 4.55 | 0.90 | 70.5% | 39.7% | 82.1% |
| Current maximum 5 upper | 56 | 5.10 | 4.35 | 0.90 | 71.4% | 41.1% | 83.9% |
| Current maximum 6 lower | 16 | 5.70 | 4.45 | 1.10 | 87.5% | 62.5% | 100.0% |
| Current maximum 6 upper or larger | 13 | 6.30 | 5.80 | 0.70 | 53.8% | 30.8% | 61.5% |

![Figure 13. Counterfactual maximum intensity for recent strong events under the 1994 year-end station geometry.](assets_en/fig13_recent_1994_counterfactual_max_intensity.png)

*Figure 13. Current observed maximum intensity and pseudo-maximum intensity under the 1994 year-end station geometry for 2011-2022 events with maximum JMA intensity 5 upper or larger. Pseudo-observations assign the nearest current report within 10 km to each 1994 year-end station. The experiment evaluates maximum-intensity sampling by station geometry and should not be interpreted as a reproduction of official 1994 observations.*
...
The 1994 year-end counterfactual for 85 recent events shows directly that observed maximum intensity itself depends on station geometry. Many events cataloged as intensity 5 upper or larger under the current network would have been sampled at a lower maximum intensity under the sparse 1994 geometry. This strengthens the interpretation of the period statistics: the post-1995 increase in maximum intensity should be read together with network densification, not as a direct change in earthquake activity alone.
...
10. Resampling 85 recent 2011-2022 events with maximum JMA intensity 5 upper or larger to the 1994 year-end station geometry gives a median maximum-intensity reduction of 0.9 units, and 83.5% of the events drop by at least one intensity class. Recent maximum-intensity statistics therefore cannot be compared directly with early-1990s statistics without accounting for network geometry.

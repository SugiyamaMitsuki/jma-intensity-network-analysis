# 観測点密度・距離減衰ベースライン比較・地域差の整理

> 注: 本レポートは前段階の密度・地域差検討を記録したアーカイブである。査読対応後の投稿原稿では，震度6弱以上を観測した63地震を対象に再集計した値を主結果として用いている。最新の投稿値は `paper/` の原稿および `data/derived/station_thinning_interpolation_6lower_plus_class/` を参照する。

## 結論

- 観測点0状態は簡略化距離減衰ベースラインのみ `gmpe_raw` として扱う。この基準の震度階級一致率は 45.5%、RMSEは 0.728。
- 全保留観測点ベースで 70%程度の震度階級一致率に達する密度は、約 57.7点/10,000 km2、最近接観測点距離中央値は約 4.35 km。ただし、これは強震域再現の十分条件ではない。
- 65%程度を許容するなら約 6.6点/10,000 km2、最近接距離中央値約 13.0 kmで到達する。68%程度なら約 23.3点/10,000 km2、最近接距離中央値約 6.9 kmが目安。
- 全保留観測点ベースの70%水準では、簡略化距離減衰ベースラインのみと比べて階級一致率は約 +25ポイント、RMSEは約 52%低下する。
- 80%および90%の震度階級一致率は、試験した密度範囲では到達しない。個別試行の最大有効密度は 71.58点/10,000 km2 だが、保持率0.9でも主要補間法の中央値は約69-70%で頭打ちとなる。
...
簡略化距離減衰ベースラインのみは、震源距離に沿った平均トレンドを与えるが、階級一致率は低く、過大評価が多い。観測点を数十点入れるだけでも空間補間は大きく改善し、最初の 0 -> 6.6点/10,000 km2 の増分で改善の大部分が得られる。一方、全保留観測点ベースで70%基準に乗せるには最近接距離4-5 km程度まで密にする必要がある。ただし強震点では70%に達せず、80%・90%基準も試験密度範囲では未到達であり、密度増加だけでなくモデル側の誤差床を下げる必要がある。

地域差は、地盤条件のばらつきだけでなく、対象イベントが内陸直下か沖合か、強震域が観測網の内側にあるか外側にあるかにも依存する。したがって、全国一律の密度しきい値は第一近似であり、複雑地形・沿岸/島嶼・沖合イベントでは局所的により高密度な観測点、または断層面距離を使った距離減衰参照場の改善が必要になる。

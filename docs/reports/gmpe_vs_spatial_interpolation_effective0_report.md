...
- `gmpe_raw` は簡略化距離減衰ベースラインからPGVを予測し震度換算するだけで、観測点によるイベント校正や残差補間を行わない。
- 主評価指標は全保留観測点に対する震度階級一致率。簡略化距離減衰ベースラインのみは約45.5%、観測点で校正した基準場は約58.8%、観測点残差を使うIDW/kriging/GMPE+krigingは高密度側で約70.3%まで改善した。
- 70%程度の階級一致率を満たすには、今回の全保留観測点ベースの点検証では約57.7点/10,000 km2、最近接観測点距離中央値で約4.35 kmが必要だった。ただし、これは強震域再現の十分条件ではない。
...
| method | 観測点の使い方 | 解釈 |
| --- | --- | --- |
| `gmpe_raw` | 0点 | 簡略化距離減衰ベースラインのみ。観測点0状態の基準。 |
| `gmpe_calibrated` | 保持観測点 | 距離減衰ベースラインをイベントごとに線形校正。空間残差は補間しない。 |
| `idw` | 保持観測点 | 地盤増幅をキャンセルした基盤面震度をIDW補間し、評価点の増幅で地表に戻す。 |
| `kriging` | 保持観測点 | 平滑化ordinary krigingで基盤面震度を補間する。 |
| `gmpe_kriging` | 保持観測点 | 距離減衰ベースラインを校正し、観測点残差をkrigingする。 |
...
80%・90%基準は密度ビン中央値では未到達である。今回の設計で試験した個別試行の最大有効密度は71.58点/10,000 km2であり、これを超える密度が少なくとも必要である。ただし高密度側の全保留観測点ベースの階級一致率は約70%で飽和しており、密度だけを上げても80%以上に届くとは解釈しない。

## 解釈

観測点0の `gmpe_raw` は、震源距離に伴う平均的な減衰トレンドを与えるが、震度階級一致率は45%程度にとどまる。過大評価率が約48%と高く、今回の震度換算・震源距離近似・断層距離近似だけでは、地震ごとの放射特性や局所的な地盤・伝播差を吸収できない。

保持観測点で距離減衰ベースラインを校正する `gmpe_calibrated` は、イベント全体の振幅バイアスを補正するため一致率が約59%に上がる。しかし密度を増やしてもほぼ頭打ちで、空間残差を使わない限り70%基準には届かない。

IDW、平滑化kriging、距離減衰残差krigingはいずれも観測点密度とともに改善し、全保留観測点ベースでは約57.7点/10,000 km2で70%基準に達する。ただし強震点では70%に達せず、80%・90%基準も未到達であり、現行手法の誤差床を下げる改善が必要である。今回のクロスバリデーションでは `gmpe_kriging` が通常krigingを明確に上回るところまでは出ていない。これは震源モデルを点震源近似、断層距離を簡略化していることも影響している可能性がある。断層面距離・Mw・断層タイプをより厳密に入れると、疎な外挿領域で距離減衰参照場の利点が出やすい。
